...
const ReaderSettings = {
    storageKey: 'maxLevelPriest_readerSettings',
...
const ChapterReader = {
    ...
    saveProgress(chapterId) {
        try {
            ...
            localStorage.setItem('maxLevelPriest_progress', JSON.stringify(progress));
        } catch (e) {
        ...
        window.addEventListener('scroll', () => {
            ...
            saveTimeout = setTimeout(() => {
                ...
                if (this.currentChapter) {
                    try {
                        const saved = JSON.parse(localStorage.getItem('maxLevelPriest_progress')) || {};
                        saved.scrollPercent = progress;
                        localStorage.setItem('maxLevelPriest_progress', JSON.stringify(saved));
                    } catch (e) { }
                }
            }, 500);
        });
